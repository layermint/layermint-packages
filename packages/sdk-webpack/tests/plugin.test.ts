...
describe('sdk-webpack plugin', () => {
  it('generates a virtual module with merged symbol exports', () => {
    ...
    try {
      ...
      writeFileSync(join(root, 'src', 'theme', 'Greeting.ts'), 'export const title = "default"\nexport const subtitle = "base"\n')
      writeFileSync(join(root, 'src', 'variants', 'tenant', 'acme', 'theme', 'Greeting.ts'), 'export const title = "tenant"\n')
      ...
      const resolveData = { request: '@/theme/Greeting' }
      beforeResolve.handler?.(resolveData)

      expect(resolveData.request).not.toBe('@/theme/Greeting')
      ...
      expect(generated).toContain('variants/tenant/acme/theme/Greeting')
      expect(generated).toContain('export { subtitle } from')
      expect(generated).toContain('src/theme/Greeting')
